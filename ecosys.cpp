...
{ 
	//14720509442452787552
	//1278357829164476668 - чемпион
				...
				if(dist < vis_dist)
				{
					...
					else
					{
						...
						else
						{
							...
							float m_ratio = (cur.m)/(vic.m);

							//если сосед труп, то не бегать от трупа, наоборот спешить быстрее, коль массивен
							if(vic.e<=0) m_ratio = necrophagia*(1.0 + vic.m/3);

							//общий множитель - если большая масса, хищничество ослабевает
							afactor *= (1 - m_ratio)*(cur_adult_m/cur.m) / (dist + 0.001);

							//непонятно, эти множители на массу слишком усиливают антагонизм
